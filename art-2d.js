...
function render2DArt(artworks) {
  ...
  container.innerHTML = `
    <div class="plant-grid">
      ${artworks.map(art => {
        // Separate program tags from other tags
        const programTags = art.tags ? art.tags.filter(tag => 
          ['procreate', 'krita', 'ibispaint'].includes(tag.toLowerCase())
        ) : [];
        const otherTags = art.tags ? art.tags.filter(tag => 
          !['procreate', 'krita', 'ibispaint'].includes(tag.toLowerCase())
        ) : [];
          ...
          <div class="plant-card art-card">
            ...
            ${programTags.length > 0 ? `
              <div class="program-badges">
                ${programTags.map(tag => `
                  <span class="program-badge" title="${tag}">
                    ${programIcons[tag.toLowerCase()] || ''}
                    ${tag}
                  </span>
                `).join('')}
              </div>
